...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  return associateLinesHandler(req, res);
};
...
async function associateLinesHandler(req: NextApiRequest, res: NextApiResponse) {  
  ...
  db.collection("users").doc(uid).collection("drafts").doc(draftId).collection("steps").doc(stepId).update({"lines": lines}); 
